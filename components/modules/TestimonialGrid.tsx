      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {layout === 'carousel' ? (
          <div className="relative max-w-4xl mx-auto">
            <div className="overflow-hidden rounded-[64px] bg-gray-50 p-12 md:p-20 border border-gray-100 shadow-2xl">
              <motion.div
                key={activeIndex}
              ...
              >
                <Quote size={80} className="mb-12 opacity-10" style={{ color: primary_color }} />
                <p className="text-2xl md:text-4xl font-medium text-gray-900 mb-16 italic leading-tight">
                  "{testimonials[activeIndex].content}"
                </p>
                
                <div className="flex flex-col items-center gap-6">
                  {testimonials[activeIndex].avatar ? (
                    <img src={testimonials[activeIndex].avatar} alt="" className="w-24 h-24 rounded-3xl object-cover shadow-2xl" referrerPolicy="no-referrer" />
                  ) : (
                    <div className="w-24 h-24 rounded-3xl bg-gray-200 flex items-center justify-center text-4xl font-black text-gray-400">
                      {testimonials[activeIndex].name.charAt(0)}
                    </div>
                  )}
                  <div>
                    <h4 className="text-2xl font-black text-gray-900">{testimonials[activeIndex].name}</h4>
                    <p className="text-xs font-black text-gray-400 uppercase tracking-[0.3em] mt-2">{testimonials[activeIndex].role}</p>
                  </div>
                </div>
              </motion.div>
            </div>
            ...
            <div className="flex justify-center gap-6 mt-12">
              <button onClick={prev} className="w-16 h-16 rounded-full bg-white border border-gray-100 flex items-center justify-center shadow-xl hover:bg-gray-900 hover:text-white transition-all">
                ←
              </button>
              <button onClick={next} className="w-16 h-16 rounded-full bg-white border border-gray-100 flex items-center justify-center shadow-xl hover:bg-gray-900 hover:text-white transition-all">
                →
              </button>
            </div>
          </div>
