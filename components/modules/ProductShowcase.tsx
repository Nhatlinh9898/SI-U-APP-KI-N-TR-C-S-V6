...
export const ProductShowcase = ({ title, subtitle, items, primary_color = '#3b82f6' }: any) => {
  const [activeGalleryIndex, setActiveGalleryIndex] = React.useState<Record<number, number>>({});
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-64">
          {items?.map((item: ProductItem, index: number) => {
            const currentImageIndex = activeGalleryIndex[index] || 0;
            const displayImage = item.gallery ? item.gallery[currentImageIndex] : item.image;
                  ...
                  {item.gallery && (
                    <div className="flex justify-center gap-4 mt-8">
                      {item.gallery.map((img, i) => (
                        <button 
                          key={i} 
                          onClick={() => setActiveGalleryIndex(prev => ({ ...prev, [index]: i }))}
                          className={`w-20 h-20 rounded-2xl overflow-hidden border-4 transition-all ${currentImageIndex === i ? 'border-blue-600 scale-110 shadow-lg' : 'border-white opacity-50 hover:opacity-100'}`}
                        >
