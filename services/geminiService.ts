...
export const generateBlueprint = async (
  ...
  try {
    ...
    prompt += `  + FeatureGrid: { title, subtitle, features: [{title, description, benefit, icon, tag, link, category, media: {type, url}, features_list: string[]}], layout: 'grid'|'list'|'bento', variant: 'card'|'glass'|'minimal' } (Lưới tính năng chi tiết với media và danh sách con)\n`;
    prompt += `  + ProductShowcase: { title, subtitle, items: [{name, description, image, gallery: string[], price, features: string[], specifications: [{label, value}], reversed: boolean, badge, rating, reviewCount, videoUrl, variants: [{label, color}]}] } (Trưng bày sản phẩm cao cấp với bộ sưu tập ảnh)\n`;
    prompt += `  + ArticleGrid: { title, subtitle, articles: [{title, excerpt, date, author: {name, avatar, role}, readingTime, image, category, tags: string[], views, isFeatured: boolean}] } (Lưới bài viết/tin tức chi tiết có lọc và tìm kiếm)\n`;
    prompt += `  + ComparisonTable: { title, subtitle, features: [{name, description, category, us: boolean|string, them: boolean|string, isHighlighted: boolean}], ourName, competitorName } (Bảng so sánh chi tiết có phân loại và làm nổi bật)\n`;
    prompt += `  + BentoGrid: { title, subtitle, items: [{title, description, icon, tag, cta_text, image, videoUrl, size: 'small'|'medium'|'large', dark: boolean, stats: {value, label}, badge, features: string[], variant: 'default'|'gradient'|'glass'}] } (Lưới Bento hiện đại với nhiều biến thể và media)\n`;
    ...
    prompt += `  + AppDownload: { title, subtitle, description, appStoreLink, playStoreLink, image, qrCode, features: string[], rating: {score, count} } (Tải ứng dụng với tính năng, đánh giá và mã QR)\n`;
    ...
    prompt += `  + RoadmapTimeline: { title, subtitle, items: [{quarter, title, description, status: 'completed'|'in-progress'|'planned', features: string[], impact, team: [{name, avatar}], milestones: [{label, date, completed}]}], variant: 'classic'|'modern' } (Lộ trình phát triển chi tiết với đội ngũ và cột mốc)\n`;
    ...
    prompt += `  + DataMetricCard: { title, subtitle, metrics: [{label, value, change, isPositive, icon, description, comparisonLabel, sparkline: number[], trend: 'up'|'down'|'stable', iconColor}] } (Thẻ chỉ số dữ liệu chuyên sâu với xu hướng và màu sắc tùy chỉnh)\n`;
    ...
    prompt += `  + TestimonialGrid: { title, subtitle, testimonials: [{name, role, content, avatar, rating, companyLogo, date}], layout: 'grid'|'carousel' } (Lưới hoặc trình trượt đánh giá khách hàng chi tiết)\n`;
    ...
    prompt += `  + PricingToggleTable: { title, subtitle, plans: [{name, monthlyPrice, yearlyPrice, features: string[], isPopular, badge, description, buttonText}] } (Bảng giá có nút gạt Tháng/Năm)\n`;
    ...
    prompt += `- Ví dụ cho FeatureGrid: "Giải pháp quản lý nhân sự 4.0", layout: "bento", variant: "glass", features: [{title: "Chấm công AI", description: "Nhận diện khuôn mặt chính xác 99.9%", media: {type: "video", url: "..."}, features_list: ["Chống giả mạo", "Tích hợp GPS"]}].\n`;
    prompt += `- Ví dụ cho ArticleGrid: "Blog Công Nghệ & Đời Sống", articles: [{title: "Tương lai của AI trong năm 2026", category: "Công nghệ", readingTime: "5 phút", views: "15.4K"}].\n`;
    prompt += `- Ví dụ cho ProductShowcase: "iPhone 15 Pro Max", price: "Từ 29.990.000đ", gallery: ["img1.jpg", "img2.jpg", "img3.jpg"], specifications: [{label: "Chip", value: "A17 Pro"}, {label: "Camera", value: "48MP"}].\n`;
