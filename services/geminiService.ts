...
export const generateBlueprint = async (
  ...
  try {
    ...
    prompt += `  + FAQAccordion: { title, subtitle, faqs: [{question, answer, category, icon, author: {name, avatar, role}, lastUpdated, relatedLinks: [{label, url}]}] } (Dùng cho trang hỗ trợ/giải đáp thắc mắc)\n`;
