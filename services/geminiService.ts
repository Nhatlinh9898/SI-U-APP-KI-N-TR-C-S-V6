...
export const generateBlueprint = async (
  ...
  try {
    ...
    prompt += `  + IntegrationDirectory: { title, subtitle, integrations: [{name, category, description, longDescription, features: string[], logo, status}] } (Danh mục tích hợp có bộ lọc và modal chi tiết)\n`;
    ...
    prompt += `  + BentoGrid: { title, subtitle, items: [{title, description, icon, tag, cta_text, image, size: 'small'|'medium'|'large'}] } (Dùng để show tính năng nổi bật kiểu Apple)\n`;
    ...
    prompt += `- Ví dụ cho TokenomicsChart: Phân bổ token cho dự án Crypto (ví dụ: "Ecosystem Growth", percentage: 40, amount: "400M Token", vesting: "4 years linear").\n`;
