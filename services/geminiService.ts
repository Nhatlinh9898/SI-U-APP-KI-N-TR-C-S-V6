...
export const generateBlueprint = async (
  ...
  try {
    ...
    prompt += `  + EventSchedule: { title, subtitle, days: [{date, title, sessions: [{time, title, speaker, speakerImage, location, description, tags: string[]}]}] } (Lịch trình sự kiện chi tiết)\n`;
    ...
    prompt += `  + ProductShowcase: { title, subtitle, items: [{name, description, image, price, features: string[], specifications: [{label, value}], reversed: boolean}] } (Trưng bày sản phẩm chi tiết)\n`;
    prompt += `  + ArticleGrid: { title, subtitle, articles: [{title, excerpt, date, author: {name, avatar}, readingTime, image, category, tags: string[]}] } (Lưới bài viết/tin tức chi tiết)\n`;
    ...
    prompt += `  + TeamSection: { title, subtitle, members: [{name, role, image, bio, expertise: string[], education, socialLinks: [{platform, url}]}] } (Đội ngũ chuyên gia chi tiết)\n`;
    ...
    prompt += `  + BentoGrid: { title, subtitle, items: [{title, description, icon, tag, cta_text, image, size: 'small'|'medium'|'large', dark, stats: {value, label}, badge}] } (Bố cục Bento hiện đại)\n`;
    prompt += `  + StatsRow: { stats: [{label, value, suffix, description, trend: {value, isUp}}] } (Hàng số liệu thống kê chi tiết)\n`;
    prompt += `  + StepByStep: { title, subtitle, steps: [{title, description, icon, duration, outcome}] } (Dùng để show quy trình chi tiết)\n`;
    prompt += `  + FeatureGrid: { title, subtitle, features: [{title, description, benefit, icon, tag, link, category}] } (Lưới tính năng chi tiết)\n`;
    prompt += `  + PricingTable: { title, subtitle, plans: [{name, price, yearlyPrice, savings, description, features: string[], unavailableFeatures: string[], isPopular: boolean, tag, targetAudience, ctaText}] } (Bảng giá dịch vụ chuyên nghiệp)\n`;
    prompt += `  + TestimonialGrid: { title, subtitle, testimonials: [{name, role, content, avatar}] }\n`;
    prompt += `  + FAQAccordion: { title, subtitle, faqs: [{question, answer, category, icon}] } (Dùng cho trang hỗ trợ/giải đáp thắc mắc)\n`;
    ...
    prompt += `- Ví dụ cho IntegrationDirectory: Hệ sinh thái ứng dụng (ví dụ: "Slack", category: "Communication", popularityScore: 4.9, reviews: 1200, author: "Official").\n`;
    ...
    prompt += `- Ví dụ cho RoadmapTimeline: Mô tả các giai đoạn phát triển sản phẩm (ví dụ: "Q4 2024", title: "Ra mắt phiên bản Mobile", features: ["iOS App", "Android App", "Push Notifications"]).\n`;
    prompt += `- Ví dụ cho SkillCloud: Hiển thị các kỹ năng chuyên môn (ví dụ: "React Native", level: 90, yearsOfExperience: 5, icon: "📱").\n`;
    prompt += `- Ví dụ cho Leaderboard: Vinh danh những người dẫn đầu (ví dụ: "Nguyễn Văn A", score: "15,400", category: "Top Contributor", achievements: ["MVP 2023", "Fast Learner"]).\n`;
    ...
    prompt += `- Ví dụ cho TimelineSection: Mô tả lịch sử hình thành hoặc lộ trình dự án (ví dụ: "Năm 2020", title: "Mở rộng thị trường Đông Nam Á", tags: ["Expansion", "Global"]).\n`;
    prompt += `- Ví dụ cho StatsRow: Sử dụng các con số tăng trưởng thực tế (ví dụ: value: "99.9", suffix: "%", label: "Uptime", trend: {value: "0.1%", isUp: true}).\n`;
    ...
    prompt += `- Ví dụ cho StepByStep: Mô tả quy trình dịch vụ chuyên nghiệp (ví dụ: "Bước 1: Khảo sát hiện trạng", duration: "2 ngày", outcome: "Bản vẽ sơ bộ").\n`;
