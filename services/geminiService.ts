...
export const generateBlueprint = async (
  ...
  try {
    ...
    prompt += `\n🚀 CHIẾN LƯỢC NỘI DUNG TỔNG THỂ (GLOBAL CONTENT STRATEGY):\n`;
    prompt += `- SỬ DỤNG "POWER WORDS": Sử dụng các từ ngữ mạnh mẽ, thôi thúc hành động (ví dụ: "Đột phá", "Độc bản", "Tối ưu", "Cam kết", "Đẳng cấp").\n`;
    prompt += `- TÍCH HỢP "SOCIAL PROOF": Trong các đoạn mô tả, hãy lồng ghép các con số chứng thực (ví dụ: "Được tin dùng bởi 500+ doanh nghiệp", "Hơn 10,000 học viên đã thành công").\n`;
    prompt += `- CÁ NHÂN HÓA THEO NGÀNH: Nếu là tiệm bánh, văn phong phải ấm cúng, gần gũi; nếu là luật sư, văn phong phải đanh thép, tin cậy; nếu là SaaS, văn phong phải hiện đại, công nghệ.\n`;
    prompt += `- CHI TIẾT ĐẾN TỪNG LABEL: Các nhãn nút (CTA) không chỉ là "Xem thêm", hãy dùng "Khám phá giải pháp", "Bắt đầu dùng thử miễn phí", "Nhận tư vấn chuyên sâu".\n`;
    prompt += `- Ví dụ cho TestimonialGrid: Viết đánh giá chi tiết, có kết quả cụ thể (ví dụ: "Nhờ giải pháp của công ty, doanh thu của chúng tôi đã tăng trưởng 40% chỉ sau 2 quý").\n`;
    prompt += `- Ví dụ cho PricingTable: Đặt tên gói dịch vụ sáng tạo (ví dụ: "Khởi nghiệp", "Bứt phá", "Thống trị") và liệt kê các tính năng mang lại giá trị thực tế cao.\n`;
    prompt += `- Ví dụ cho HeroSection: Tiêu đề phải là một lời hứa giá trị (Value Proposition) mạnh mẽ, không chỉ là tên công ty.\n`;
